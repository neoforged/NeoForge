            ...
            try {
                ReflectionUtils.addListener(helper, new GameTestListener() {
                    ...
                    public void testFailed(GameTestInfo info, GameTestRunner runner) {
                        framework.changeStatus(test, Test.Status.failed("GameTest fail: " + info.getError().getMessage(), info.getError()), null);
                        disable();
                    }
                ...
                try {
                    ...
                    ((MutableTestFramework) framework).tests().setStatus(test.id(), Test.Status.failed("GameTest fail: " + assertion.getMessage(), assertion));
                    throw assertion;
                }
