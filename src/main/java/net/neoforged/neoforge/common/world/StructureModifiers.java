...
public final class StructureModifiers {
     ...
     * @param spawners   List of SpawnerData's specifying EntityType, weight, and pack size.
     */
    public record AddSpawnsStructureModifier(HolderSet<Structure> structures,
                                             List<MobSpawnSettings.SpawnerData> spawners) implements StructureModifier {

        /**
         * Convenience method for using a single SpawnData.
         *
         * @param structures Structures to add mob spawns to.
         ...
         */
        public static AddSpawnsStructureModifier singleSpawn(HolderSet<Structure> structures, MobSpawnSettings.SpawnerData spawner) {
        ...
        public void modify(Holder<Structure> structure, Phase phase, ModifiableStructureInfo.StructureInfo.Builder builder) {
            if (phase == Phase.ADD && this.structures.contains(structure)) {
                ...
                this.spawners.forEach(spawnerData -> {
                    MobCategory category = spawnerData.type.getCategory();
                    structureSettings.getOrAddSpawnOverrides(category).addSpawn(spawnerData);
                });
            }
        }
        ...
        public void modify(Holder<Structure> structure, Phase phase, ModifiableStructureInfo.StructureInfo.Builder builder) {
            if (phase == Phase.REMOVE && this.structures.contains(structure)) {
                ...
                Arrays.stream(MobCategory.values()).forEach(mobCategory -> {
                    var spawnOverrides = structureSettings.getSpawnOverrides(mobCategory);
                    if (spawnOverrides == null || spawnOverrides.getSpawns().isEmpty())
                        return;

                    spawnOverrides.removeSpawns(spawnerData -> this.entityTypes.contains(BuiltInRegistries.ENTITY_TYPE.getHolderOrThrow(ResourceKey.create(Registries.ENTITY_TYPE, new ResourceLocation(NeoForgeVersion.MOD_ID, BuiltInRegistries.ENTITY_TYPE.getKey(spawnerData.type).getPath())))));
                });
            }
        }
